...
description: "Autonomous visual porting skill v3.4. Pixel-perfect design cloning from any public URL into a Next.js target via MCP browser. Uses programmatic scroll-driven testing (NOT screenshots) to capture and verify every animation, interaction, and visual state. Runtime behavior recording at 21 scroll positions captures computed transforms, video currentTime, nav state, and element visibility. Automated dual-site comparison ensures zero visual deviation. Adapts to target architecture (monorepo, i18n, Tailwind v3/v4). Detects actual source animation patterns (native RAF, CSS keyframes, IO, GSAP, Lenis) and replicates the EXACT implementation — never substitutes libraries. Network profile extraction for CDN-loaded libraries. Section inventory extraction with parity enforcement. Modern CSS detection (dvh, @starting-style, color-mix, text-stroke). Zero hardcoded values. Single command, zero manual input."
---
