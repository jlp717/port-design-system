...
Parse `the source path, target path, and optional commit hash provided by the user` as:
...
- If the platform did not pass `the source path, target path, and optional commit hash provided by the user`, recover the same values from the user request.
