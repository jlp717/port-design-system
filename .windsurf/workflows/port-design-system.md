<!-- AUTO-GENERATED from .claude/skills/port-design-system/SKILL.md — do not edit directly.
     Run `node scripts/sync-skills.mjs` to regenerate. -->


# Port Design System

You are about to port the complete visual design system from a **source project** to a **target project** using the paths provided in `the source and target paths provided by the user`.

Parse `the source and target paths provided by the user` as two filesystem paths separated by a space. The first path is the **source** (the design you want to copy). The second path is the **target** (your production project that will receive the new look). If either path contains spaces, the user should wrap it in quotes.
...
1. **Parse arguments.** Split `the source and target paths provided by the user` into `<source-path>` and `<target-path>`. If fewer than 2 paths are provided, ask the user for the missing path. Resolve relative paths to absolute.
