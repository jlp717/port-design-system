---
description: "CLON LITERAL PIXEL-PERFECT v5.0. DS-FIRST: before generating any code, exhaustively analyzes the design system repo (components, tokens, hooks, animation patterns) and creates a section→DS-component map. Uses EXCLUSIVELY existing DS components and tokens — never copies source class names. RAW extraction of fullHTML + allCSS + keyframes + scrollData + assets + JS behaviors. Programmatic numeric verification: getBoundingClientRect + getComputedStyle of ALL visible elements, 21 scroll positions, 3 viewports. Absolute honesty about fidelity gaps. Visual difference < 0.5% or FAIL."
