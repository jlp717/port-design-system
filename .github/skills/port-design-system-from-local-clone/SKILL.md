...
description: "CLON LITERAL PIXEL-PERFECT v4.0. Clonador web obsesivo: el target debe ser INDISTINGUIBLE del source en todos los viewports (1920x1080, 768x1024, 375x812), en las 21 posiciones de scroll, en todas las animaciones, hover, focus y estados. Tolerancia cero. Extraccion RAW de fullHTML + allCSS + keyframes + scrollData + assets + JS behaviors — SIN screenshots estaticos como fuente de verdad. Analisis numerico con getBoundingClientRect + getComputedStyle de TODOS los elementos visibles (sin limite). Diferencia visual < 0.5% medida numericamente. Assets hotlinkeados del source (prompts IA solo si tienen branding incrustado). Checklist bloqueante mostrado en cada respuesta. 23 scripts de extraccion RAW + FASE 0.5 obligatoria. Un unico comando, cero atajos, cero aproximaciones."
---
