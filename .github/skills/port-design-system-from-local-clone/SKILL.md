...
description: Port the full visual design system from a LOCAL source clone folder into an existing Next.js target project while preserving target content and business logic. Use when the user asks to transplant look-and-feel, animations, tokens, and assets from one local project into another. Requires three arguments: <source-path> <target-path> <commit-hash>.
---
