...
description: "CLON LITERAL PIXEL-PERFECT v5.0. DS-FIRST: antes de generar cualquier código, analiza exhaustivamente el repositorio del design system (componentes, tokens, hooks, animation patterns) y crea un mapa sección→componente DS. Usa EXCLUSIVAMENTE componentes y tokens del DS — nunca class names del source. Extracción RAW de fullHTML + allCSS + keyframes + scrollData + assets + JS behaviors. Verificación programática numérica: getBoundingClientRect + getComputedStyle de todos los visibles, 21 scroll positions, 3 viewports. Honestidad absoluta sobre gaps de fidelidad. Fidelidad < 0.5% o FAIL."
---
