...
description: Perform full visual replacement of an existing Next.js target project using a local Next.js source clone as the only visual truth. Use when invoked as /port-design-system-from-local-clone "<source-path>" "<target-path>" or /port-design-system-from-local-clone "<source-path>" "<target-path>" "<commit-hash>" in Codex or Cursor, when the goal is to audit both codebases, optionally checkout a target baseline with safe stash handling, extract the complete source visual system literally, install missing visual dependencies, copy source-owned visual assets, and apply the source design language across every target route while preserving target text content, content data, routes, and business logic.
---
