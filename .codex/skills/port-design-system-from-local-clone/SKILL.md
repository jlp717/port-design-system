...
description: Port the COMPLETE visual design system (EVERY visual pixel, layout, animation, token, font, color, spacing, hover state, scroll effect) from a LOCAL source clone folder into an existing Next.js target project. Target keeps its content, routes, and business logic but looks IDENTICAL to source visually. Requires two arguments: <source-path> <target-path>. Optional: <commit-hash>.
...
1. **Parse arguments.** Split `$ARGUMENTS` into `<source-path>`, `<target-path>`, and optionally `<commit-hash>`. If fewer than 2 paths are provided, ask the user for the missing path. Resolve relative paths to absolute.
