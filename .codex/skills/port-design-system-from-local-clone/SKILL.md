...
description: Literal visual transplant of a Next.js source clone into a target repo. DESIGN 100% from source (copied verbatim, never rewritten). TEXT 100% from target (all user-visible strings preserved). Opens dual Chrome DevTools MCP instances (reference-url + target dev server) before touching any file and keeps both open until done. Third argument is public reference URL (e.g. jobyaviation.com) for pre-flight source validation. Requires PAGE_MAPPING.md before any code. Every visual file is COPIED then text-swapped, never rewritten or refactored. Build runs after every single file. Migration is complete only when pixel delta <= 1.5% at all scroll positions in both MCP instances for all pages.
---
