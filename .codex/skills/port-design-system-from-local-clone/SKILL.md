...
description: "Autonomous visual porting skill v3.1. Clones the complete design system from any public URL into a Next.js target. Extracts tokens, animations, 3D scenes, assets, interactions, dark mode, accessibility, and responsive behavior via MCP browser. Handles cross-origin CSS, Shadow DOM, CSS-in-JS, pseudo-elements, @property, @supports, Adobe Fonts, custom scrollbars, CSS Motion Path, Lottie/Rive/Spline, container queries, scroll-timeline, View Transitions, and every modern CSS/JS feature. Zero hardcoded values — all design comes from extraction. Single command, zero manual input."
---
...
const _metadata = { extractedAt: new Date().toISOString(), sourceUrl: location.href,
  ...
  scrollHeight: document.documentElement.scrollHeight, skillVersion: '3.1' };
```
...
RIVE_ANIMATION, SPLINE_3D, SCROLL_TIMELINE_CSS, VIEW_TIMELINE_CSS,
VIEW_TRANSITION_API, DIALOG_MODAL, POPOVER, LOTTIE_DOTLOTTIE,
SCROLL_DRIVEN_ANIM, CONTAINER_QUERY_ANIM, ANCHOR_SCROLL,
CSS_NESTING_ANIM, SUBGRID_LAYOUT, MASONRY_LAYOUT,
...
**Tailwind v4 (CSS-first config)**: NO existe `tailwind.config.js`.
Los tokens se definen directamente en CSS con `@theme`:
...
### 4.4.2 Verificacion de stacking contexts
