...
description: Port the entire visual design system from a source website project to a target Next.js production project — transfers CSS tokens, typography, animations, GSAP configs, Lenis smooth scroll, parallax effects, responsive breakpoints, and component visual shells while preserving the target's business logic, content, routing, and integrations. Use this whenever the user wants to apply, port, transfer, migrate, or transplant a design system from one project to another. Also triggers on phrases like "apply this design to my project", "transfer the look and feel", "skin my app with this design". Provide source and target paths as arguments.
...
You are about to port the complete visual design system from a **source project** to a **target project** using the paths provided in `$ARGUMENTS`.

Parse `$ARGUMENTS` as two filesystem paths separated by a space. The first path is the **source** (the design you want to copy). The second path is the **target** (your production project that will receive the new look). If either path contains spaces, the user should wrap it in quotes.
...
1. **Parse arguments.** Split `$ARGUMENTS` into `<source-path>` and `<target-path>`. If fewer than 2 paths are provided, ask the user for the missing path. Resolve relative paths to absolute.

2. **Validate source.** Confirm `<source-path>` exists and contains a recognizable web project:
   - Has `package.json` — OR —
...
3. **Validate target.** Confirm `<target-path>` exists and contains a Next.js project:
...
4. **Report findings.** Before proceeding, display to the user:
```
Source: <source-path>
  Framework: <detected>
  Styling: <detected>
...
5. **Create working directories.** Create `docs/port-design-system/` in the target project if it doesn't exist. This is where extraction reports and changelogs will be written.

6. **Verify target builds.** Run the target's build command (`npm run build` or equivalent from its `package.json` scripts). If it fails, tell the user to fix their build first. A broken target before porting begins means any post-port failures will be impossible to attribute.
